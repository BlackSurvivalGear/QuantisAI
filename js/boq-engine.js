(function (global) {
  ...
  const sourceOf = (item, fallback = {}) => {
    ...
    return {
      ...
      page: page || null,
      calculation: src.calculation || item?.calculation || null,
  ...
  function explicitMeasurement(item, fallbackSource = {}) {
    ...
    if (type === 'area' || type === 'rectangle' || (n(item.length) !== null && n(item.width) !== null && !n(item.height))) {
      const length = n(item.length), width = n(item.width), quantity = rectangleArea(length, width);
      if (quantity !== null) return { quantity, unit: item.unit || 'm²', source, formula: `${length} × ${width}` };
    }
    if (type === 'volume') {
      const length = n(item.length), width = n(item.width), height = n(item.height ?? item.depth);
      if ([length, width, height].every(v => v !== null && v > 0)) return { quantity: round(length * width * height, 3), unit: item.unit || 'm³', source, formula: `${length} × ${width} × ${height}` };
    }
  ...
  function fromDrawingInterpretation(drawing) {
    ...
    const measurements = [], clarifications = [];
  ...
  function parseRevision(text, fallback) {
    const match = text.match(/(?:drawing\s+)?revision\s*[:\-]?\s*([A-Z0-9-]+)/i) || text.match(/\brev\.?\s*[:\-]?\s*([A-Z0-9-]+)/i);
    return match ? match[1].trim() : fallback || null;
  }

  function parseDrawingNumber(text, fallback) {
    const match = text.match(/(?:drawing\s*(?:number|no\.?|reference)?|sheet)\s*[:\-]?\s*([A-Z]{1,3}-?\d{2,4})/i) || text.match(/\b(A-?\d{3})\b/i);
    return match ? match[1].trim().replace(/^A(?=\d)/i, 'A-') : fallback || null;
  }
  ...
  function parseControlData(text) {
    ...
    const patterns = {
      ...
      rooms: /\brooms\s*[:\-]?\s*(\d+)/i,
      doors: /\bdoors\s*[:\-]?\s*(\d+)/i,
      windows: /\bwindows\s*[:\-]?\s*(\d+)/i
    };
    Object.entries(patterns).forEach(([key, pattern]) => {
      const match = text.match(pattern); if (!match) return;
      controls[key] = key === 'overallFootprint' ? { length: Number(match[1]), width: Number(match[2]) } : Number(match[1]);
    });
    return controls;
  }

  function compareControl(label, actual, expected, tolerance = 0.01) {
    if (expected === undefined || expected === null || actual === null || actual === undefined) return null;
    const difference = round(Number(actual) - Number(expected), 3);
    return { label, actual: Number(actual), expected: Number(expected), pass: Math.abs(difference) <= tolerance, difference };
  }

  function fromExtractedText(text, fallbackSource = {}) {
    if (typeof text !== 'string' || !text.trim()) return null;
    const drawing = parseDrawingNumber(text, fallbackSource.drawing || null);
    const revision = parseRevision(text, fallbackSource.revision || null);
    const source = { drawing, revision, page: 1 };
    const controls = parseControlData(text);
    ...
    const length = Number(overallMatch[1]), width = Number(overallMatch[2]), wallHeight = Number(wallHeightMatch[1]);
    if (!(length > 0 && width > 0 && wallHeight > 0)) return null;

    const page2Index = text.search(/(?:^|\n)\s*(?:Page\s*2\b|A-?201\s+ROOF\s*\/\s*SECTION|ROOF\s*\/\s*SECTION\s*\/\s*CONSTRUCTION)/i);
    const page1Text = page2Index >= 0 ? text.slice(0, page2Index) : text;
    const page2Text = page2Index >= 0 ? text.slice(page2Index) : '';

    const openings = [];
    const openingRegex = /\b(D\d+|W\d+)\b[^\n\r]*?(\d+(?:\.\d+)?)\s*[×x]\s*(\d+(?:\.\d+)?)/gi;
    let openingMatch;
    while ((openingMatch = openingRegex.exec(page1Text)) !== null) {
      const id = openingMatch[1].toUpperCase(), openingWidth = Number(openingMatch[2]), openingHeight = Number(openingMatch[3]);
      if (openingWidth > 0 && openingHeight > 0) openings.push({ id, type: id.startsWith('D') ? 'door' : 'window', width: openingWidth, height: openingHeight, source: { ...source, page: 1 } });
    }

    const roomDefinitions = [
      ['LIVING / DINING', /LIVING\s*\/\s*DINING/], ['KITCHEN', /KITCHEN/], ['MASTER BED', /MASTER\s+BED(?:ROOM)?/], ['ENSUITE', /ENSUITE/],
      ['BEDROOM 2', /BEDROOM\s*2/], ['BEDROOM 3', /BEDROOM\s*3/], ['STUDY', /STUDY/], ['UTILITY', /UTILITY/]
    ];
    const rooms = [];
    roomDefinitions.forEach(([name, labelPattern]) => {
      const match = page1Text.match(new RegExp(labelPattern.source + '\\s+(\\d+(?:\\.\\d+)?)\\s*m?\\s*[×x]\\s*(\\d+(?:\\.\\d+)?)\\s*m?', 'i'));
      if (match) rooms.push({ name, length: Number(match[1]), width: Number(match[2]), source: { ...source, page: 1 }, evidenceConfidence: 1 });
    });

    const structuralElements = [];
    const structuralRegex = /^\s*(S\d+)\b([^\n\r]*?)(\d+(?:\.\d+)?)\s*m\b/i;
    page2Text.split(/\r?\n/).forEach(line => {
      const match = line.match(structuralRegex);
      if (match) structuralElements.push({ id: match[1].toUpperCase(), description: match[2].trim(), length: Number(match[3]), unit: 'm', source: { ...source, page: 2 }, evidenceConfidence: 1 });
    });

    const constructionInfo = {};
    const knownConstructionKeys = new Set(['External wall', 'Floor', 'Roof', 'Insulation', 'Ceiling', 'Internal partitions', 'Finishes']);
    page2Text.split(/\r?\n/).forEach(line => {
      const match = line.match(/^\s*([^:]{2,35}):\s*(.+)$/); if (!match) return;
      const key = match[1].trim(); if (knownConstructionKeys.has(key)) constructionInfo[key] = match[2].trim();
    });

    const perimeter = 2 * (length + width);
    const data = {
      stage: 'drawing-interpreter', status: 'success', confidence: 1, source, pagesProcessed: page2Index >= 0 ? 2 : 1,
      rooms, openings,
      walls: [{ external: true, length: perimeter, height: wallHeight, openings, source: { ...source, page: 1 }, evidenceConfidence: 1 }],
      structuralElements, constructionInfo, mechanicalSystems: [], electricalSystems: [],
      measurements: [
    ...
    const roomArea = rooms.reduce((sum, room) => sum + rectangleArea(room.length, room.width), 0);
    const wallGross = rectangleArea(perimeter, wallHeight);
    const wallNet = wallNetArea(perimeter, wallHeight, openings);
    const openingDeduction = wallGross === null || wallNet === null ? null : round(wallGross - wallNet, 3);
    data.controlValidation = {
      ...
      roomCount: compareControl('Rooms', rooms.length, controls.rooms, 0),
      doorCount: compareControl('Doors', openings.filter(o => o.type === 'door').length, controls.doors, 0),
      windowCount: compareControl('Windows', openings.filter(o => o.type === 'window').length, controls.windows, 0),
      floorArea: compareControl('Floor area', roomArea, controls.floorArea),
      externalWallLength: compareControl('External wall length', perimeter, controls.externalWallLength),
      grossExternalWallArea: compareControl('Gross external wall area', wallGross, controls.grossExternalWallArea),
      openingDeduction: compareControl('Opening deduction', openingDeduction, controls.openingDeduction),
      netExternalWallArea: compareControl('Net external wall area', wallNet, controls.netExternalWallArea)
    };
  ...
  function buildBoQ(takeoff) {
    ...
    const items = measurements.filter(m => Number.isFinite(m.quantity) && m.quantity >= 0).map((m, i) => ({
      itemNo: m.itemNo || `BQ-${String(i + 1).padStart(3, '0')}`, section: m.section || 'Unclassified', description: m.description, location: m.location || null,
      unit: m.unit, quantity: m.quantity, rate: null, amount: null, source: m.source, calculation: m.calculation,
      assumptions: m.state === STATES.INFERRED ? ['Source drawing reference incomplete; quantity derived from explicit dimensions but requires QS verification.'] : [], state: m.state, confidence: m.confidence
    }));
  ...
  function insufficient(stage, reason, extra = {}) {
    const base = { stage, status: 'failed', reasonCode: 'INSUFFICIENT_DATA', reason, confidence: null, ...extra };
