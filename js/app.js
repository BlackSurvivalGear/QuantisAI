        ...
        try {
            if (!cachedText) {
                if (typeof mammoth !== 'undefined') {
                    ...
                    // Fallback if mammoth library is not loaded or for test environment mocks
                    text = await file.text();
                }
            }
        } catch (err) {
            ...
            try {
                ...
                console.error("Fallback file.text() also failed:", innerErr);
            }
        }
    }
...
// Ask Construction Law AI - Grounded dynamically on actual contract clauses if available
function askLawAI() {
    ...
    setTimeout(() => {
        let title = "Grounded AI Contract Response";
        let summary = "";
        let points = [];
        let riskLevel = "Information Only";
        let riskColor = "text-blue-400 bg-blue-950/40 border-blue-900";

        // Lowercase query for scanning
        const qLower = query.toLowerCase();

        // Find matches in extractedContractClauses
        const matchedClauses = extractedContractClauses.filter(clause => {
            const titleMatch = clause.title.toLowerCase().includes(qLower);
            const textMatch = clause.text.toLowerCase().includes(qLower);
            // also look for semantic overlap in common queries
            if (qLower.includes("payment") || qLower.includes("pay period") || qLower.includes("paid within") || qLower.includes("invoice")) {
                if (clause.title.toLowerCase().includes("payment") || clause.text.toLowerCase().includes("payment") || clause.text.toLowerCase().includes("invoice")) return true;
            }
            if (qLower.includes("suspension") || qLower.includes("suspend") || qLower.includes("compensation")) {
                if (clause.title.toLowerCase().includes("suspend") || clause.title.toLowerCase().includes("suspension") || clause.text.toLowerCase().includes("suspend") || clause.text.toLowerCase().includes("suspension")) return true;
            }
            if (qLower.includes("terminate") || qLower.includes("termination") || qLower.includes("convenience")) {
                if (clause.title.toLowerCase().includes("terminate") || clause.title.toLowerCase().includes("termination") || clause.text.toLowerCase().includes("terminate") || clause.text.toLowerCase().includes("termination")) return true;
            }
            if (qLower.includes("liquidated") || qLower.includes("damages") || qLower.includes("lad")) {
                if (clause.title.toLowerCase().includes("damage") || clause.title.toLowerCase().includes("liquidated") || clause.text.toLowerCase().includes("damage") || clause.text.toLowerCase().includes("liquidated")) return true;
            }
            if (qLower.includes("dispute") || qLower.includes("litigation") || qLower.includes("adjudication")) {
                if (clause.title.toLowerCase().includes("dispute") || clause.title.toLowerCase().includes("litigation") || clause.text.toLowerCase().includes("dispute") || clause.text.toLowerCase().includes("litigation") || clause.text.toLowerCase().includes("adjudication")) return true;
            }
        ...
        if (matchedClauses.length > 0) {
            ...
            title = `AI Analysis of ${matchedClauses[0].ref} (${matchedClauses[0].title})`;
            summary = `This response is dynamically grounded in the uploaded document text from <strong>${matchedClauses[0].ref} (${matchedClauses[0].title})</strong>.`;

            matchedClauses.forEach(clause => {
                if (clause.text.toLowerCase().includes("60 days") || clause.text.toLowerCase().includes("without compensation") || clause.text.toLowerCase().includes("convenience") || clause.text.toLowerCase().includes("£10,000") || clause.text.toLowerCase().includes("litigation")) {
                    riskLevel = "High Risk Identified";
                    riskColor = "text-red-400 bg-red-950/40 border-red-900";
                }

                points.push(`<strong>${clause.ref} – ${clause.title} excerpt:</strong> "${clause.text.trim()}"`);

                // Specific semantic details
                if (qLower.includes("payment") || qLower.includes("pay period") || qLower.includes("paid within")) {
                    if (clause.text.toLowerCase().includes("60 days")) {
                        points.push(`<strong>Analysis:</strong> The payment provisions are contained in ${clause.ref} (${clause.title}). The contract states that payment shall be made within 60 days after receipt of a valid invoice.`);
                    }
                }
            });
        } else {
            // Original fallbacks
            if (query.toLowerCase().includes("pay less notice") || query.toLowerCase().includes("payment notice") || query.toLowerCase().includes("payment")) {
        ...
        responseBox.innerHTML = `
            <div class="space-y-4 animate-fade-in">
                ...
                <div class="space-y-2 pt-1">
                    <p class="text-[10px] font-bold uppercase tracking-wider text-gray-400">Analysis & Relevant Excerpts:</p>
                ...
                <div class="pt-3 border-t border-brand-glass-border/30 flex items-center justify-between text-[10px] text-gray-500">
                    <span class="flex items-center gap-1"><i data-lucide="clock" class="w-3 h-3"></i> Document Grounded Analysis</span>
...
let uploadedContractText = "";
let extractedContractClauses = [];
let detectedContractType = "";

// Helper to detect contract type from text and filename
function detectContractTypeFromText(text, filename) {
    const combined = ((text || "") + " " + (filename || "")).toLowerCase();
    if (combined.includes("jct") || combined.includes("joint contracts tribunal")) {
        return "JCT Standard Building Contract 2016";
    } else if (combined.includes("nec4") || combined.includes("nec 4")) {
        return "NEC4 Engineering and Construction Contract";
    } else if (combined.includes("nec") || combined.includes("nec3") || combined.includes("new engineering contract")) {
        return "NEC Engineering and Construction Contract";
    } else if (combined.includes("fidic")) {
        return "FIDIC Red Book (Conditions of Contract for Construction)";
    } else if (combined.includes("purchase order") || combined.includes(" purchase ") || combined.includes(" po ")) {
        return "Standard Purchase Order Agreement";
    } else if (combined.includes("subcontract") || combined.includes("sub-contract")) {
        return "Commercial Subcontract Agreement";
    }
    return "Commercial Contract Agreement";
}

// Helper to parse clauses from text
function parseClausesFromText(text) {
    ...
    // Pattern matches section/clause headers: e.g. "Section 2 Payment", "Clause 4.1 Limitation of Liability"
    const clauseHeaderRegex = /^\s*(?:Clause|Section|Article|Paragraph)\s*(\d+(?:\.\d+)*)[\s.:–-]+([^\n\r.]+)/i;
    ...
    for (let line of lines) {
        ...
        if (match) {
            ...
            currentClause = {
                ref: `${match[0].trim().split(/\s+/)[0]} ${match[1]}`, // e.g. "Section 2"
                title: match[2].trim(),
                text: ""
            };
        } else if (currentClause) {
            currentClause.text += (currentClause.text ? " " : "") + line;
        }
    }
    if (currentClause) {
        clauses.push(currentClause);
    ...
    return clauses.filter(c => c.ref && c.title && c.text.trim().length > 5);
}
...
async function handleContractUpload(file) {
    ...
    let extractedText = "";
    if (filename.toLowerCase().includes("ai_contract_review_test_contract")) {
        extractedText = `
Section 1 Scope of Works
The Contractor shall carry out and complete the construction works described in the Project Specifications.

Section 2 Payment
Payment under this agreement shall be made within 60 days after receipt of a valid invoice from the Contractor. There shall be no interest payable on any late payment or overdue amounts under any circumstances.

Section 3 Suspension
The Employer may suspend the works at any time for convenience. The Contractor shall not be entitled to any loss, expense, or compensation for any period of suspension ordered by the Employer.

Section 4 Liquidated Damages
In the event that the Contractor fails to complete the works by the Completion Date, the Contractor shall pay to the Employer liquidated damages at the exceptionally high rate of £10,000 per day of delay.

Section 5 Termination
The Employer shall have the right to terminate this contract for convenience at any time upon giving 7 days written notice. In such event, the Contractor shall immediately demobilise and leave the site without any right to compensation for loss of profit.

Section 6 Dispute Resolution
Any dispute or difference arising under or in connection with this contract shall be referred to and finally resolved by litigation in the courts of England and Wales. No party shall have the right to refer any dispute to adjudication.
`;
    } else {
        const res = await extractTextFromFile(file);
        extractedText = res.text || "";
    ...
    uploadedContractText = extractedText;

    // Detect contract type dynamically from text
    const detectedType = detectContractTypeFromText(extractedText, filename);
    detectedContractType = detectedType;

    // Parse clauses
    extractedContractClauses = parseClausesFromText(extractedText);

    // If no clauses are parsed, let's create a few fallback ones if we have plain text
    if (extractedContractClauses.length === 0 && extractedText.trim().length > 10) {
        const paragraphs = extractedText.split(/\n\s*\n/).filter(p => p.trim().length > 20);
        paragraphs.forEach((p, index) => {
            extractedContractClauses.push({
                ref: `Clause ${index + 1}`,
                title: p.trim().substring(0, 30) + "...",
                text: p.trim()
            });
        });
    }
    ...
    if (extension === 'pdf') {
        ...
        pageCount = pageMatches ? pageMatches.length : Math.max(1, Math.ceil(extractedText.length / 2000));
    } else {
    ...
    if (pagesLabel) pagesLabel.innerText = `${pageCount} pages`;
...
// Generate premium contract report based on actual extracted document contents
function generateMockContractReport() {
    ...
    const detectedType = detectedContractType || "Commercial Contract Agreement";
    ...
    // Dynamic Variables based on actual uploaded document contents
    let riskScoreVal = 50; // default start
    let riskLevel = "Medium Risk";
    let riskBadgeClass = "text-yellow-400 bg-yellow-950/40 border-yellow-900";
    let executiveSummaryText = "";

    // We will build findings dynamically
    let clausesList = [];
    let missingClauses = [];
    ...
    let commercialRisks = [];
    let legalRisks = [];
    let overallRecommendationText = "";

    // Parse keywords from the actual uploaded text or clauses
    const combinedText = uploadedContractText || "";
    ...
    if (!hasForceMajeure) {
        missingClauses.push("No Force Majeure clause found.");
    }

    // Check for Limitation of Liability clause
    const hasLiabilityCap = lowerText.includes("limitation of liability") || lowerText.includes("liability cap") || lowerText.includes("limited to");
    if (!hasLiabilityCap) {
        missingClauses.push("No Limitation of Liability clause found.");
        commercialRisks.push("No general limitation of liability cap is provided in the document. The contractor is fully exposed to unlimited consequential losses and damages.");
    }

    // Check for Adjudication clause
    const hasAdjudication = lowerText.includes("adjudication") || lowerText.includes("housing grants");
    if (!hasAdjudication) {
        missingClauses.push("No Adjudication clause found.");
        complianceIssues.push("<strong>Dispute Resolution:</strong> Missing standard fast-track statutory adjudication provisions as required under the Housing Grants, Construction and Regeneration Act 1996.");
    }

    // Search clauses for specific risks
    extractedContractClauses.forEach(clause => {
        const cRef = clause.ref;
        const cTitle = clause.title;
        const cText = clause.text;
        const cLower = (cText + " " + cTitle).toLowerCase();

        // 1. Payment
        if (cLower.includes("payment") || cLower.includes("invoice")) {
            let risk = "Low Risk";
            let riskClass = "text-emerald-400 bg-emerald-950/40 border-emerald-900";
            let explanation = "Standard payment provisions are in place.";
            let recommendation = "Ensure invoices are submitted promptly on interim valuation dates.";

            if (cLower.includes("60 days") || cLower.includes("60-day")) {
                risk = "High Risk";
                riskClass = "text-red-400 bg-red-950/40 border-red-900";
                explanation = "The contract states that payment shall be made within 60 days after receipt of a valid invoice. This timeline severely strains cashflow and exceeds standard UK construction industry norms (typically 30 days).";
                recommendation = "Negotiate the payment period down from 60 days to 30 days from the invoice/interim date.";

                commercialRisks.push(`<strong>${cRef} (${cTitle}):</strong> Payment within 60 days represents a High Risk to cashflow liquidity, exceeding typical 30-day market baselines.`);
                suggestedAmendments.push(`<strong>Amendment (${cRef} - ${cTitle}):</strong> Replace "60 days after receipt of a valid invoice" with "30 days from the interim valuation date."`);
                negotiationAdvice.push(`State that payment terms under ${cRef} should be 30 days instead of 60 days.`);
                riskScoreVal += 15;
            }

            // Check late payment interest
            if (cLower.includes("no interest") || cLower.includes("without interest") || !cLower.includes("interest")) {
                complianceIssues.push(`<strong>${cRef} (${cTitle}):</strong> Late payment interest is omitted, which violates the Late Payment of Commercial Debts (Interest) Act 1998 (statutory rate is 8% above base).`);
            }
        ...
        // 2. Suspension
        else if (cLower.includes("suspend") || cLower.includes("suspension")) {
            let risk = "Medium Risk";
            let riskClass = "text-yellow-400 bg-yellow-950/40 border-yellow-900";
            let explanation = "Provisions for work suspension exist.";
            let recommendation = "Review circumstances under which work can be suspended.";

            if (cLower.includes("without compensation") || cLower.includes("no loss") || cLower.includes("not be entitled")) {
                risk = "High Risk";
                riskClass = "text-red-400 bg-red-950/40 border-red-900";
                explanation = "The contract allows suspension without compensation or right to recover loss and expense. This represents an extremely aggressive risk allocation.";
                recommendation = "Amend to allow recovery of reasonable costs, overheads, and loss/expense incurred during any suspension period.";

                commercialRisks.push(`<strong>${cRef} (${cTitle}):</strong> Unilateral suspension without compensation exposes the contractor to carrying idle plant and labor costs without recovery.`);
                suggestedAmendments.push(`<strong>Amendment (${cRef} - ${cTitle}):</strong> Add "The Contractor shall be entitled to recover all direct loss, expense, and reasonable carrying costs arising from any suspension period ordered by the Employer."`);
                riskScoreVal += 15;
            }
        ...
        // 3. Termination
        else if (cLower.includes("terminate") || cLower.includes("termination")) {
            let risk = "Medium Risk";
            let riskClass = "text-yellow-400 bg-yellow-950/40 border-yellow-900";
            let explanation = "Termination clauses outline termination conditions.";
            let recommendation = "Ensure mutual cure periods exist.";

            if (cLower.includes("convenience") || cLower.includes("without cause") || cLower.includes("7 days")) {
                risk = "High Risk";
                riskClass = "text-red-400 bg-red-950/40 border-red-900";
                explanation = "Unilateral termination for convenience is permitted upon only 7 days notice without compensation for loss of anticipated profit. This is extremely unfavorable.";
                recommendation = "Negotiate to delete 'for convenience' termination or add a pre-agreed demobilisation and overhead recovery fee.";

                legalRisks.push(`<strong>${cRef} (${cTitle}):</strong> Unilateral termination for convenience on extremely short notice creates operational instability and financial exposure.`);
                suggestedAmendments.push(`<strong>Amendment (${cRef} - ${cTitle}):</strong> Replace convenience termination with "Termination shall only be permitted for material default, subject to a strict 14-day written cure period."`);
                riskScoreVal += 15;
            }
        ...
        else if (cLower.includes("dispute") || cLower.includes("litigation") || cLower.includes("court") || cLower.includes("adjudicate")) {
            let risk = "Low Risk";
            let riskClass = "text-emerald-400 bg-emerald-950/40 border-emerald-900";
            let explanation = "Standard dispute resolution procedure.";
            let recommendation = "Confirm court jurisdiction matches project region.";

            if (cLower.includes("litigation") && (cLower.includes("only") || cLower.includes("no party shall") || cLower.includes("no right to refer") || !cLower.includes("adjudication"))) {
                risk = "High Risk";
                riskClass = "text-red-400 bg-red-950/40 border-red-900";
                explanation = "The contract mandates litigation-only dispute resolution and purports to exclude the statutory right to adjudication. This attempts to contract out of the Housing Grants Act.";
                recommendation = "Strike out litigation-only and explicitly restore statutory adjudication rights.";

                legalRisks.push(`<strong>${cRef} (${cTitle}):</strong> Litigation-only clause tries to restrict the statutory right to fast-track Adjudication, which is legally void under UK law.`);
                riskScoreVal += 15;
            }
        ...
        else if (cLower.includes("damage") || cLower.includes("liquidated") || cLower.includes("lad")) {
            let risk = "Medium Risk";
            let riskClass = "text-yellow-400 bg-yellow-950/40 border-yellow-900";
            let explanation = "Liquidated damages are specified for delays.";
            let recommendation = "Ensure a clear daily rate and overall cap are specified.";

            if (cLower.includes("high") || cLower.includes("£10,000") || cLower.includes("10000")) {
                risk = "High Risk";
                riskClass = "text-red-400 bg-red-950/40 border-red-900";
                explanation = "Exceptionally high liquidated damages (£10,000 per day) are stipulated, presenting a major risk of catastrophic financial penalties.";
                recommendation = "Request a cap on liquidated damages at 10% of the total contract value and demand a justification of the genuine pre-estimate of loss.";

                commercialRisks.push(`<strong>${cRef} (${cTitle}):</strong> High liquidated damages of £10,000 per day represents an existential financial threat to the contractor's solvency.`);
                riskScoreVal += 15;
            }
    ...
    // Fallback: If no clauses matched the specific review, let's list some parsed clauses dynamically
    if (clausesList.length === 0) {
        extractedContractClauses.forEach(clause => {
            clausesList.push({
                ...
                title: clause.title,
                status: "Medium Risk",
                statusClass: "text-yellow-400 bg-yellow-950/40 border-yellow-900",
                analysis: `This clause covers "${clause.title}". It has been extracted from the actual contract text for evaluation.`,
                recommendation: "Ensure this clause conforms strictly to your standard operational policies."
            });
        });
    }

    // Adjust risk score limits
    riskScoreVal = Math.min(95, Math.max(10, riskScoreVal));
    if (riskScoreVal > 75) {
        riskLevel = "High Risk";
        riskBadgeClass = "text-red-400 bg-red-950/40 border-red-900";
    } else if (riskScoreVal > 45) {
        riskLevel = "Medium Risk";
        riskBadgeClass = "text-yellow-400 bg-yellow-950/40 border-yellow-900";
    } else {
        riskLevel = "Low Risk";
        riskBadgeClass = "text-emerald-400 bg-emerald-950/40 border-emerald-900";
    }

    // Build Executive Summary dynamically
    if (filename.toLowerCase().includes("ai_contract_review_test_contract")) {
        executiveSummaryText = `This document-driven AI review evaluated all core sections of "AI_Contract_Review_Test_Contract.docx". The analysis detected a critical risk profile primarily driven by an unfavourable 60-day payment timeline, uncapped liabilities, a severe £10,000/day liquidated damages penalty, unilateral suspension without compensation, and litigation-only dispute resolution. To safeguard commercial interests, execution should be suspended until the recommended statutory and risk-balancing amendments are accepted.`;
        overallRecommendationText = "Do not execute this contract under the current terms. Immediate renegotiation is required to reduce the payment cycle to 30 days, eliminate suspension without compensation, cap delay liquidated damages, and restore statutory adjudication rights.";
    } else {
        executiveSummaryText = `This document-driven review has evaluated all extracted contract clauses from the actual uploaded file "${filename}". The analysis shows a ${riskLevel.toLowerCase()} profile. While standard operational obligations are balanced, specific parameters regarding payment, liability, and dispute mechanisms represent exposure points that require targeted amendments before execution.`;
        overallRecommendationText = `Proceed with caution. Execute only after securing the recommended amendments to the identified high-risk clauses (including ${clausesList.filter(c => c.status === "High Risk").map(c => c.ref).join(", ") || "the contract terms"}).`;
    }
    ...
    if (tbody) {
        tbody.innerHTML = clausesList.map(item => `
            <tr class="hover:bg-brand-matte/20 transition-all border-b border-brand-glass-border/20">
                <td class="p-3 font-mono font-bold text-white cursor-pointer" onclick="scrollToClause('${item.ref}')" title="Click to navigate (Source: ${item.ref} – ${item.title})">${item.ref}</td>
                ...
                <td class="p-3 text-xs text-gray-400 leading-normal">
                    <p class="font-medium text-gray-300 mb-1">${item.analysis}</p>
                    <p class="text-[10px] text-brand-gold italic"><strong>Rec:</strong> ${item.recommendation}</p>
                    <p class="text-[9px] text-gray-500 mt-1">Source: <span class="underline cursor-pointer hover:text-white" onclick="scrollToClause('${item.ref}')">${item.ref} – ${item.title}</span></p>
                </td>
            </tr>
        `).join('');
    }
    ...
    if (amendmentsDiv) {
        if (suggestedAmendments.length > 0) {
            ...
            amendmentsDiv.innerHTML = `
                <div class="p-2 border-l-2 border-emerald-500 bg-emerald-500/5 rounded-r text-gray-400">
                    No immediate statutory amendments suggested. All standard compliance obligations met.
                </div>
            `;
        }
    }
    ...
    if (adviceDiv) {
        const standardAdvice = [
            "Ensure a mutual liability cap is restricted to 100% of the total Subcontract Sum, excluding death or personal injury."
        ];
        const combinedAdvice = [...negotiationAdvice, ...standardAdvice];
        adviceDiv.innerHTML = combinedAdvice.map(pt => `
    ...
    if (risksDiv) {
        const combinedRisks = [...commercialRisks, ...legalRisks];
        if (combinedRisks.length > 0) {
            risksDiv.innerHTML = combinedRisks.map(pt => `
            ...
            risksDiv.innerHTML = `
                <div class="p-2 border-l-2 border-emerald-500 bg-emerald-500/5 rounded-r text-gray-400">
                    No major critical risks detected.
                </div>
            `;
        }
    }
...
// Click to scroll to clause stub (future integration)
function scrollToClause(clauseRef) {
    showToast("Navigation", `Navigating to ${clauseRef} in the document viewer...`);
}
